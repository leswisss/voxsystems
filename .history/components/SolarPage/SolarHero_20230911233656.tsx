...
const SolarHero = ({setState, state, state2}: {setState: React.Dispatch<React.SetStateAction<boolean>>, state: boolean, state2: boolean}) => {
    ...
    <section className="solar">
      <div className="container solar-hero__container">
        ...
        <div className="s-hero__image" ref={heroRefs[7]}>
              ...
              <Image src={image} alt="Solar Energy" onLoadingComplete={() => setState(false)} fill={true} sizes='(max-width: 600px) 100%, 100%' key={`si-${i}`} style={{opacity: activeImage === 0 ? 1 : 0}} />
              <Image src={image} alt="Solar Energy" onLoadingComplete={() => setState(false)} fill={true} sizes='(max-width: 600px) 100%, 100%' key={`si-${i}`} style={{opacity: activeImage === 1 ? 1 : 0}} />
        </div>
      </div>
    </section>
  )
