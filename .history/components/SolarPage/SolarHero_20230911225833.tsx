        ...
        <div className="s-hero__image" ref={heroRefs[7]}>
          {
            heroImages.map((image, i) => (
              <Image src={image} alt="Solar Energy" {i===1 && onLoadingComplete={() => setState(false)}} fill={true} sizes='(max-width: 600px) 100%, 100%' key={`si-${i}`} style={{opacity: activeImage === i ? 1 : 0}} />
            ))
          }
        </div>
      </div>
