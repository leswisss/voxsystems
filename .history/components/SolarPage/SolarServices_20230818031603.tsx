...
const SolarServices = () => {
  const [activeAccordion, setActiveAccordion] = useState({active: true, selected: 0})
          ...
          <div className="accordion__container">
            {
              SolarAccordionData.map((item, i) => (
                <SolarAccordion key={`sac-${i}`} data={item} state={activeAccordion} setState={setActiveAccordion}/>
              ))
            }
            <SolarAccordion />
          </div>
