...
const SolarHero = ({setState, state, state2}: {setState: React.Dispatch<React.SetStateAction<boolean>>, state: boolean, state2: boolean}) => {
  ...
  useEffect(() => {
    ...
    tl2
      ...
      !(!state && !state2) ? tl2.pause() : tl.play()
        ...
        <div className="s-hero__image"  ref={heroRefs[7]}>
          {
            heroImages.map((image, i) => (
              <Image src={image} alt="Solar Energy" fill={true} sizes='(max-width: 600px) 100%, 100%' key={`si-${i}`} style={{opacity: activeImage === i ? 1 : 0}} onLoadingComplete={() => setState(false)}/>
            ))
          }
        </div>
      </div>
