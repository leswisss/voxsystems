...
import { slideFooterAnim, slideYFooterAnim, slideLeftFooterAnim, slideLinkFooterAnim } from "@/animations";
        ...
        <div className="footer__top">
          ...
          <div className="f-top__content">
            <div className="f-top__left">
              ...
              <motion.div variants={slideLeftFooterAnim} animate={inView ? "entry" : "initial"} className="newsletter">
                <input
                  type="email"
                  value={emailValue}
            ...
            <div className="f-top__right">
              {FooterData.map((item, i) => (
                <div className="footer__links" key={`ff-${i}`}>
                  <motion.h4 variants={slideFooterAnim} animate={inView ? "entry" : "initial"}>{item.name}</motion.h4>

                  <div className="f-links__container">
                    {item.links.map((link, i) => (
                      <motion.div className="f-link-wrap"  key={`f-${i}`} custom={i} variants={slideLinkFooterAnim} animate={inView ? "entry" : "initial"}>
                        <Link href={link.href}>
                          {link.name}
                        </Link>
                      </motion.div>
