...
const TeamHero = ({setState, state, state2}: {setState: React.Dispatch<React.SetStateAction<boolean>>, state: boolean,  state2: boolean}) => {
  ...
  useEffect(() => {
    ...
    tl.to(heroRefs[0].current, { y: "0%", duration: 1, ease: "power4" })
    ...
    !(!state && !state2) ? tl.pause() : tl2.play()
  }, [state, state2]);
