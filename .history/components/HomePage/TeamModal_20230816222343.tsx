...
const TeamModal = ({team , activeModal}: ModalProps) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mousemove", (e) => {
      xMoveContainer(e.clientX - modalRef.current?.offsetWidth / 2)
      yMoveContainer(e.clientY - modalRef.current?.offsetHeight / 2)

      xMoveCursor(e.clientX -  modalRef.current?.offsetWidth / 2)
      yMoveCursor(e.clientY -  modalRef.current?.offsetHeight / 2)
    })
  }, [])
