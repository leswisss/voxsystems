  ...
  return (
    <Link href="/notre-equipe/swiss" ref={ref} className='team-data__container' key={`td-${data.id}`} onMouseEnter={() => setActiveTeam({ active: true, index: data.id-1 })} onMouseLeave={() => setActiveTeam({ active: false, index: data.id-1 })}>
      <div className="user__status">
        <motion.span variants={slideYFooterAnim} >{data.post}</motion.span>
      </div>
      <div className="user__name-status">
        <motion.div variants={slideYFooterAnim}  className="user__name">{data.name}</motion.div>
        <motion.div variants={slideYFooterAnim}  className="user__status-2">{data.post2}</motion.div>
      </div>
    </Link>
  )
}
