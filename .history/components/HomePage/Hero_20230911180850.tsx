...
const Hero = ({setState, state}: {setState: React.Dispatch<React.SetStateAction<boolean>>, state: boolean}) => {
  ...
  useEffect(() => {
    ...
    tl2
      .to(textRef[0].current, {y: "0%", duration: 1, ease: "power4"}, 0.2)
      .to(textRef[1].current, {y: "0%", duration: 1, ease: "power4"}, 0.2)
      .to(textRef[2].current, {y: "0%", duration: 1, ease: "power4"}, 0.2)
      .to(footerRef.current, {y: "0px", duration: 1, ease: "power4"}, 0.5)

    state ? tl2.pause() : tl2.play()
