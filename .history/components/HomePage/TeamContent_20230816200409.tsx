...
const TeamContent = ({ data, activeTeam, setActiveTeam, manageModal}: TeamProps) => {
  return (
    <Link href="/" className='team-data__container' key={data.id} onMouseEnter={(e) => {manageModal(true, data.id - 1, e.clientX, e.clientY)}} onMouseLeave={(e) => {manageModal(false, index, e.clientX, e.clientY)}} >
