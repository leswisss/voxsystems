...
const TeamContent = ({ data, activeTeam, setActiveTeam}: TeamProps) => {
  return (
    <Link href="/" className='team-data__container' key={data.id} onMouseEnter={() => setActiveTeam({ active: true, index: data.id-1 })} onMouseLeave={() => setActiveTeam({ active: true, index: data.id-1 })}>
