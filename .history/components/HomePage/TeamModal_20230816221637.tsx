...
const TeamModal = ({team , activeModal}: ModalProps) => {
  ...
  useEffect(() => {
    //Move Container
    let xMoveContainer = gsap.quickTo(modalRef.current, "center", {duration: 0.8, ease: "power3"})
    let yMoveContainer = gsap.quickTo(modalRef.current, "top", {duration: 0.8, ease: "power3"})
    //Move cursor
    let xMoveCursor = gsap.quickTo(cursorRef.current, "center", {duration: 0.5, ease: "power3"})
