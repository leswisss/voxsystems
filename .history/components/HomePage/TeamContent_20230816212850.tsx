...
interface ManageModal {
  (active: boolean, index: number, x , y:): void;
}
...
const TeamContent = ({ data, activeTeam, setActiveTeam, manageModal}: TeamProps) => {
  return (
    <Link href="/" className='team-data__container' key={data.id} onMouseEnter={(e) => {manageModal(true, data.id - 1)}} onMouseLeave={(e) => {manageModal(false, data.id - 1)}} >
