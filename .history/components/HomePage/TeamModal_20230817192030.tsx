...
const TeamModal = ({team , activeModal}: ModalProps) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      if (modalRect.current && cursorRect.current) {
        const x = e.clientX - modalRect.left;
        const y = e.clientY - modalRect.top;

        const x1 = e.clientX - cursorRect.left;
        const y1 = e.clientY - cursorRect.top;

        // Center the item on the mouse position
      gsap.to(modalRect.current, {
        x: x - modalRect.current.offsetWidth / 2,
        y: y - modalRect.current.offsetHeight / 2,
        duration: 0.1
      });
      }
