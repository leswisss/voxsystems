...
const Services = () => {
  ...
  const overflowRefs = HeroData.map(() => useRef<HTMLDivElement>(null));
  const cardRefs = HeroData.map(() => useRef<HTMLDivElement>(null));


  useEffect(() => {
    gsap.registerPlugin(ScrollTrigger);
  
    overflowRefs.forEach((overflowRef, i) => {
      gsap.from(cardRefs[i].current, {
        scrollTrigger: {
          trigger: overflowRef.current,
          start: "top+=150px bottom",
          end: "top+=220px bottom",
          scrub: 0.3,
          markers: true,
          repea
        },
      ...
      <div className="container services__container">
        ...
        <div className="services__wrapper">
          {HeroData.map((item, i) => (
            ...
            >
              <div className="service__card" ref={cardRefs[i]}>
