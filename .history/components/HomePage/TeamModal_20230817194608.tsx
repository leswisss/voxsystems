...
const TeamModal = ({team , activeModal}: ModalProps) => {
  ...
  useEffect(() => {
    //Move Container
    let xMoveContainer = gsap.quickTo(modalRef.current, "x", {duration: 0.8, ease: "power3"})
    let yMoveContainer = gsap.quickTo(modalRef.current, "y", {duration: 0.8, ease: "power3"})
    //Move cursor
    let xMoveCursor = gsap.quickTo(cursorRef.current, "x", {duration: 0.5, ease: "power3"})
    let yMoveCursor = gsap.quickTo(cursorRef.current, "y", {duration: 0.5, ease: "power3"})
