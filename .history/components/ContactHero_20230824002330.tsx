      ...
      <div className="selection__box">
        {
          ContactSelection.map((item, i) => (
            <Link href={`/contacts?service=${item.slug}`} key={`sb-${i}`} onClick={() => handleClick(i)} className={activeContact===i ? "active-span" : ""}>{item.name} CONTACT</Link>
          ))
        }
      </div>
